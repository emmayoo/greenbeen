...
<!-- 		<h2 class="text-primary">게시글 상세정보</h2> -->
		...
		<c:if test="${position=='master'}">
			<a href="ad_best_modifyForm.env?id=ad_best_list&pageNum=${pageNum}&num=${board.num}"
			class="btn btn-info">수정</a>
			<a href="ad_best_deleteForm.env?id=ad_best_list&pageNum=${pageNum}&num=${board.num}"
			class="btn btn-info">삭제</a> 
		</c:if>
		<c:if test="${position=='friend'}">
			<a href="ad_best_replyForm.env?id=ad_best_list&pageNum=${pageNum}&num=${board.num}"
