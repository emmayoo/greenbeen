...
<div class="container"> 
		...
		<c:if test="${position=='master'}">
			<a href="files_modifyForm.env?id=files_list&pageNum=${pageNum}&num=${board.num}"
			class="btn btn-info">수정</a> 
			<a href="files_deleteForm.env?id=files_list&pageNum=${pageNum}&num=${board.num}"
			class="btn btn-info">삭제</a> 
		</c:if>
		<c:if test="${position=='friend'}">
			<a href="files_replyForm.env?id=files_list&pageNum=${pageNum}&num=${board.num}"
