			...
			<c:if test="${not empty list}">
				...
				<c:forEach var="board" items="${list }">
					<tr>
						...
						<c:if test="${board.del !='y' }">
							<td>${board.writer}</td>
							<td><a href="know_quiz_view.env?id=know_quiz_list&pageNum=1&num=${board.num}"> 
									${board.subject} 
		...
		<form action="know_quiz_list.env?id=know_quiz_list&pageNum=1">
			...
			<c:if test="${not empty keyword}">
				...
				<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
					...
					<a href="know_quiz_list.env?id=know_quiz_list&pageNum=${i}&search=${search}&keyword=${keyword}">[${i}]</a></li>
				</c:forEach>
				<c:if test="${pp.endPage < pp.totalPage}">
					<li><a href="know_quiz_list.env?id=know_quiz_list&pageNum=${pp.endPage + 1}&search=${search}&keyword=${keyword}">[다음]</a></li>
				</c:if>
			</c:if>
			<c:if test="${empty keyword}">
				<c:if test="${pp.startPage > pp.pagePerBlk }">
					<li><a href="know_quiz_list.env?id=know_quiz_list&pageNum=${pp.startPage - 1}">이전</a></li>
				</c:if>
				<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
