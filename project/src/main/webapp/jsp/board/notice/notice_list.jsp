...
<div class="container"> 
		...
		<tr><td >공지사항 새 글:${na}</td>
			<td style="text-align:right">
				<c:if test="${position=='master'}">
	 			<div align=right> 		
				<a class="btn btn-light notice_new">게시글 작성</a>
				</div>
				</c:if>
			</td>
		</tr>
			...
			<c:if test="${not empty list}">
				...
				<c:forEach var="board" items="${list }">
					<tr>
						...
						<c:if test="${board.del !='y' }">
							<td>${board.writer}</td>
							<td><a href="notice_view.env?id=notice_list&pageNum=${pageNum}&num=${board.num}"> 
									${board.subject} 
		...
		<form action="notice_list.env?pageNum=1">
		...
		<ul class="pagination pagination-sm">
			<c:if test="${not empty keyword}">
				<c:if test="${pp.startPage > pp.pagePerBlk }">
					<li><a href="notice_list.env?id=notice_list&pageNum=${pp.startPage - 1}&search=${search}&keyword=${keyword}">[이전]</a></li>
				</c:if>
				<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
					<li <c:if test="${pp.currentPage==i}"> class="active" </c:if>>
					<a href="notice_list.env?id=notice_list&pageNum=${i}&search=${search}&keyword=${keyword}">[${i}]</a></li>
				</c:forEach>
				<c:if test="${pp.endPage < pp.totalPage}">
					<li><a href="notice_list.env?id=notice_list&pageNum=${pp.endPage + 1}&search=${search}&keyword=${keyword}">[다음]</a></li>
				</c:if>
			</c:if>
			<c:if test="${empty keyword}">
				<c:if test="${pp.startPage > pp.pagePerBlk }">
					<li><a href="notice_list.env?id=notice_list&pageNum=${pp.startPage - 1}">이전</a></li>
				</c:if>
				<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
					<li <c:if test="${pp.currentPage==i}">class="active"</c:if>><a
						href="notice_list.env?id=notice_list&pageNum=${i}">${i}</a></li>
				</c:forEach>
				<c:if test="${pp.endPage < pp.totalPage}">
					<li><a href="notice_list.env?id=notice_list&pageNum=${pp.endPage + 1}">다음</a></li>
				</c:if>
			</c:if>
		</ul></div>
