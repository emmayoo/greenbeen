...
<input type="hidden" value="${id}" id="hidden" name="id">
		...
		<tr><td >질문있어요 새 글:${na}</td>
			<td style="text-align:right">
	 			<div align=right> 		
				<a class="btn btn-light question_new">질문 작성</a>
				</div>
			</td>
		</tr>
		</table>
		...
		<form action="question_list.env?id=question_list&pageNum=${pageNum}">
		...
		<ul class="pagination pagination-sm">
			<c:if test="${not empty keyword}">
				<c:if test="${pp.startPage > pp.pagePerBlk }">
					<li><a href="question_list.env?id=question_list&pageNum=${pp.startPage - 1}&search=${search}&keyword=${keyword}">[이전]</a></li>
				</c:if>
				<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
					<li <c:if test="${pp.currentPage==i}"> class="active" </c:if>>
					<a href="question_list.env?id=question_list&pageNum=${i}&search=${search}&keyword=${keyword}">[${i}]</a></li>
				</c:forEach>
				<c:if test="${pp.endPage < pp.totalPage}">
					<li><a href="question_list.env?id=question_list&pageNum=${pp.endPage + 1}&search=${search}&keyword=${keyword}">[다음]</a></li>
				</c:if>
			</c:if>
			<c:if test="${empty keyword}">
				<c:if test="${pp.startPage > pp.pagePerBlk }">
					<li><a href="question_list.env?id=question_list&pageNum=${pp.startPage - 1}">이전</a></li>
				</c:if>
				<c:forEach var="i" begin="${pp.startPage}" end="${pp.endPage}">
					<li <c:if test="${pp.currentPage==i}">class="active"</c:if>><a
						href="question_list.env?id=question_list&pageNum=${i}">${i}</a></li>
				</c:forEach>
				<c:if test="${pp.endPage < pp.totalPage}">
					<li><a href="question_list.env?id=question_list&pageNum=${pp.endPage + 1}">다음</a></li>
				</c:if>
			</c:if>
		</ul></div>
