...
<script type="text/javascript">
	...
	function up(id) { //확인버튼 눌렀을 때
		var replytext = $('#tt_'+id).val();
		var formData = "rno="+id+'&replytext='+replytext
			+"&bno=${board.num}";
		$.post('question_repUpdate.env',formData, function(data) {
			$('#slist').html(data);
		});
	}
	function lst() { //취소버튼을 눌렀을 때
		$('#slist').load('question_slist.env?num=${board.num}'); //ReplyBoardController.java에서 넘어온 board값
	}
	
	function del(rno,bno) {
		var formData="rno="+rno+"&bno="+bno;
		$.post("question_repDelete.env",formData, function(data) {
			$('#slist').html(data);
		});
	}
</script>
...
<body>
	<div class="container" align="center">
		...
		<table class="table table-bordered">
			...
			<c:forEach var="rb" items="${slist}"> <!--//ReplyBoardController.java에서 넘어온 slist값  -->
				<tr>
					<td>${rb.replyer}</td>
					<td id="td_${rb.rno}"><pre>${rb.replytext}</pre></td>
					<td>${rb.updatedate }</td>
					<td id="btn_${rb.rno}">
						<c:if test="${rb.replyer==sessionScope.id }">
