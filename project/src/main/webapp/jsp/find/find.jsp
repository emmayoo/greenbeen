...
<script>
	function find(){
	 ...
	 function more(){
		 ...
		 alert(find_val);
		 location.href="find.env?find_val="+find_val+"&more=com";
	 }
</script>
<style>
	.highlight {
...
<div id="content" style="margin-left:30%">
	...
	<c:if test="${not empty com_list}" >
		<h4>회사 검색 (총 ${com_cnt}개) <c:if test="${com_cnt>10}"><a onclick="more()">더보기</a></c:if></h4>
		<%-- <c:forEach var="com" items="${com_list}" begin='0' end='9'>
			<a onclick="com_name(event)" href="#">${com.com_name}</a><br>
		</c:forEach> --%>
		<c:forEach var="com" items="${com_list}" begin='0' end='9'>
			<a onclick="com_name(event)" href="#">${com}</a><br>
		</c:forEach>
	</c:if>
	
	<!--자료실 -->
	<c:if test="${not empty files_list}" >
		<h4>자료실 (총 ${files_cnt}개) <c:if test="${files_cnt>10}"><a href="find.env?find_val=${find_val}&more=files">더보기</a></c:if></h4>
	...
	<c:if test="${not empty notice_list}" >
		<h4>공지사항 (총 ${notice_cnt}개) <c:if test="${notice_cnt>10}"><a href="find.env?find_val=${find_val}&more=notice">더보기</a></c:if></h4>
	...
	<c:if test="${not empty ad_list}">
		<h4>회사홍보 (총 ${ad_cnt}개) <c:if test="${ad_cnt>10}"><a href="find.env?find_val=${find_val}&more=ad">더보기</a></c:if></h4>
	...
	<c:if test="${not empty news_list}">
		<h4>오늘의 뉴스 (총 ${news_cnt}개) <c:if test="${news_cnt>10}"><a href="find.env?find_val=${find_val}&more=news">더보기</a></c:if></h4>
	...
	<c:if test="${not empty q_list}">
		<h4>질문 있어요 (총 ${q_cnt}개) <c:if test="${q_cnt>10}"><a href="find.env?find_val=${find_val}&more=question">더보기</a></c:if></h4>
